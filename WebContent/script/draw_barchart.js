...
var PRICE_TOP = 20;       // draw above this height
...
function updateEndIndex(reddotPosX) {
//alert("update: " + reddotPosX);
	...
	if (reddotPosX < 0 || xStep == 0) {
		endIndex = dataSize - 1;
	} else {
		endIndex = Math.round((reddotPosX + halfReddotW - firstBarX - chartX) / xStep + startIndex - 0.2);
		
		//alert("endindex: " + endIndex);
		// do not move out to the limits
	...
	jg.fillRect(300, 300, 200, 30);
	jg.setColor(colorWhite);
	jg.drawString("Last Date: " + dateArray[endIndex], 320, 310);
	jg.paint();
}
...
//alert("draw it: " + xMove);
	...
	// find what ema's
	if (form.doEMA9.checked == true) {
		doEMA9 = true;
	} else {
		doEMA9 = false;
	}
	
	if (form.doEMA21.checked == true) {
		doEMA21 = true;
	} else {
		doEMA21 = false;
	}
	...
	// move the reddot to the position
	moveReddot(endIndex);
}
...
function drawBars() {
	...
	var xPolylineAvg21  = new Array();
	var yPolylineAvg21 	= new Array();
	var xPolylineAvg9  	= new Array();
	var yPolylineAvg9 	= new Array();
	...
	var totalPointsAvg21 = 0;
	var totalPointsAvg9 = 0;
	
	var volumeHt = 0;
	
//	alert("start/end: " + startIndex + "/" + endIndex + ", xStep: " + xStep);
	for (var i = startIndex; i <= endIndex; i ++, xx += xStep) {
		yyOpen 	= Math.round(origY - PRICE_BOTTOM - yStep * (Math.log(openArray[i]) - minPLog)) ;
		yyClose = Math.round(origY - PRICE_BOTTOM - yStep * (Math.log(closeArray[i]) - minPLog));
		yyMin 	= Math.round(origY - PRICE_BOTTOM - yStep * (Math.log(minArray[i]) - minPLog));
		yyMax 	= Math.round(origY - PRICE_BOTTOM - yStep * (Math.log(maxArray[i]) - minPLog));			
		...
		if (doEMA50) {
			if (logAvg50Array[i] <= maxPLog && logAvg50Array[i] >= minPLog) {				
				...
				yPolylineAvg50[totalPointsAvg50] = Math.round(origY - PRICE_BOTTOM - yStep * (logAvg50Array[i] - minPLog));
				totalPointsAvg50++;
			}
		}
		
		if (doEMA21) {
			if (logAvg21Array[i] <= maxPLog && logAvg21Array[i] >= minPLog) {				
				xPolylineAvg21[totalPointsAvg21] = roundXX;
				yPolylineAvg21[totalPointsAvg21] = Math.round(origY - PRICE_BOTTOM - yStep * (logAvg21Array[i] - minPLog));
				totalPointsAvg21++;
			}
		}
		
		if (doEMA9) {
			if (logAvg9Array[i] <= maxPLog && logAvg9Array[i] >= minPLog) {				
				xPolylineAvg9[totalPointsAvg9] = roundXX;
				yPolylineAvg9[totalPointsAvg9] = Math.round(origY - PRICE_BOTTOM - yStep * (logAvg9Array[i] - minPLog));
				totalPointsAvg9++;
			}
		}
	...
	if (doEMA21) {
		jg.setColor(colorBlue);
		jg.drawPolyline(xPolylineAvg21, yPolylineAvg21);
	}
	
	if (doEMA9) {
		jg.setColor(colorGreen);
		jg.drawPolyline(xPolylineAvg9, yPolylineAvg9);
	}
...
function moveReddot(positionIndex) {
	var xPos = Math.round(chartX + firstBarX + (positionIndex - startIndex) * xStep) - halfReddotW;
	dd.elements.reddot.moveTo(xPos, reddotY);
}
...
//alert("prepare data");
	...
	for (var i = 0; i < dataArray.length; i ++) {
		...
		openArray[i] 		= parseFloat(nodeInfo[1]);
		closeArray[i] 		= parseFloat(nodeInfo[2]);
		minArray[i] 		= parseFloat(nodeInfo[3]);
		maxArray[i] 		= parseFloat(nodeInfo[4]);
		logAvg9Array[i] 	= parseFloat(nodeInfo[5]);
		logAvg21Array[i] 	= parseFloat(nodeInfo[6]);
		logAvg50Array[i] 	= parseFloat(nodeInfo[7]);
