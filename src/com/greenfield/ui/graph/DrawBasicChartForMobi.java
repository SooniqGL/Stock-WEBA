...
//import com.greenfield.ui.service.LinearModelService;
//import com.greenfield.ui.service.RSIService;
 ...
 */
public class DrawBasicChartForMobi extends BaseGraphics {
	private static final Logger LOGGER = Logger.getLogger(DrawBasicChartForMobi.class); 
	...
	// MEASURE CONSTANTS
	public static final int DEFAULT_IMAGE_WIDTH = 450;
	public static final int DEFAULT_IMAGE_HEIGHT = 280;

	private int X_LEFT = 25;
	private int X_RIGHT = 8;
	private int Y_TOP = 8;
	private int Y_BOTTOM = 8;
	private int VOLUME_HEIGHT = 25;    // for volume box
	private int GROWTH_HEIGHT = 15;    // for GROWTH box
	private int FORCE_HEIGHT = 15;     // for FORCE box
	private int RS_HEIGHT = 15;
	private int TITLE_HEIGHT = 15;
	private int BOX_GAP = 4;
	private int SCALE_LENGTH = 3;      // bars in y axis
	private int BAR_WIDTH = 2;         // bar width in price chart
	private int PRICE_TOP = 10;       // draw above this height
	private int PRICE_BOTTOM = 10;       // draw above this height
	private int TEXT_HEIGHT = 10;
	private int VERTICAL_TEXT_X1 = 17;   // price
	private int SMALL_GAP = 3;
	...
	private void drawChartFrame() {
		...
		String title = "";
                ...
                if (stock.getCompanyName() != null && stock.getCompanyName().length() > 20) {
                    companyName = companyName.substring(0, 20);
                }

		if (type != null && type.equals(SMALL_TYPE)) {
			title = companyName + " (" + stock.getTicker() + "), " + SHORT_COPYRIGHT;
		} else {
			title = companyName + " (" + stock.getTicker() + "), " + SHORT_COPYRIGHT;
		}
		...
		pad.transform(at);
		pad.setColor(colorBlue);
		pad.drawString("Vol", - volumeY, VERTICAL_TEXT_X1);
		...
		pad.drawString("Chart - " + currentDate, - origY + BOX_GAP, VERTICAL_TEXT_X1);
		pad.drawString("Gt", 	- growthY, 	VERTICAL_TEXT_X1);
		pad.drawString("Fc", - forceY,  	VERTICAL_TEXT_X1);
		if (doRSI) {
	...
	private void drawBarChart() throws Exception {
		...
		// System.out.println("barTop: " + barTop + ", barBottom: " + barBottom);
		...
		if (priceList.size() - startIndex < 110) {
			barrulerStep = 5;    // regular = 3
		} else if (priceList.size() - startIndex < 180) {
			barrulerStep = 5;   // regular = 6
		...
		for (int i = startIndex; i < priceList.size(); i ++, xx += xStep) {
			...
			//System.out.println("avg50Y: " + avg50Y + ", avg100Y: " + avg100Y);
			...
			if (doZone) {
				...
				if (daily.getAverage100() > 0) {
					...
					if ((avg100Y != barTop || avg50Y != barTop) &&
						(avg100Y != barBottom || avg50Y != barBottom)) {
			...
			} else if (daily.getSlope() <= 0 && daily.getAcceleration() <= 0) {
				pad.setColor(colorRed);
				pad.fillOval((int)xx - 2, (int) (growthY + 2), 4, 4);
				//pad.drawRect((int)xx, (int) (growthY - growthHt), (int) xStep + 1, (int) (growthHt - zeroGrowthHt));
			} else { // if (daily.getSlope() <= 0 && daily.getAcceleration() <= 0) {
		...
		pad.setColor(chartColor);
		pad.fillRect(origX + SMALL_GAP, volumeStrY - TEXT_HEIGHT - 2, volumeStrLen + 8, TEXT_HEIGHT + 5);
		pad.setColor(colorBlack);
		pad.drawRect(origX + SMALL_GAP, volumeStrY - TEXT_HEIGHT - 2, volumeStrLen + 8, TEXT_HEIGHT + 5);
	...
	private void displayMonthScale(Vector monthList) {
		...
		for (int i = 0; i < monthList.size(); i ++) {
                    if (monthList.size() > 12 && (i % 4 != 0)) {
                        continue;  // skip one
                    } else if (monthList.size() > 6 && (i % 2 == 1)) {
                        continue;  // skip one
                    }
	...
	private void drawLineList(Vector barList) {
		...
		for (int i = 0; i < barList.size(); i ++) {
			...
			pad.drawLine(line.getStartX(), line.getStartY(),
			...
			if (stockId != null) {
				...
				if (stkList != null && stkList.size() > 0) {
					...
					stockId = stock.getStockId();
                        ...
                        // try to get service context from Pool, doRSI = N, O - for weekly option
			ServiceContext svrContext = DBCachePool.getServiceContext(stock, period, displayDate, "O", "N", stockDao);
			if (svrContext != null) {
