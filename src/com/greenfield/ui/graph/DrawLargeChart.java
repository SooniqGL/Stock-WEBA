 ...
 */
public class DrawLargeChart extends BaseGraphics {
	private static final Logger LOGGER = Logger.getLogger(DrawBasicChart.class);
	...
	public static final int DEFAULT_IMAGE_HEIGHT = 745;
	public static final int DEFAULT_IMAGE_HEIGHT_NO_OPTION = 615;
	

	private int X_LEFT = 36;
	private int X_RIGHT = 12;
	private int Y_TOP = 8;
	private int Y_BOTTOM = 12;
	private int VOLUME_HEIGHT = 71; // 40; // for volume box
	private int GROWTH_HEIGHT = 53; // 30; // for GROWTH box
	private int FORCE_HEIGHT = 53; // 30; // for FORCE box
	private int RS_HEIGHT = 53;  // 30
	private int TITLE_HEIGHT = 35; // 20;
	private int BOX_GAP = 8;
	private int SCALE_LENGTH = 6; // bars in y axis
	private int BAR_WIDTH = 4; // bar width in price chart
	private int PRICE_TOP = 30; // draw above this height
	private int PRICE_BOTTOM = 28; // draw above this height
	private int TEXT_HEIGHT = 23; // 15;
	private int VERTICAL_TEXT_X1 = 23; // price
	private int SMALL_GAP = 9;
	...
	public void init() {
		...
		imageColor = new Color(225, 225, 225);
		chartColor = new Color(255, 255, 248);
		...
		// for the MA lines
		thickStroke = new BasicStroke(2.9f, BasicStroke.CAP_BUTT, BasicStroke.CAP_ROUND);
	...
	public void paint() throws Exception {
		...
		if (priceList.size() - startIndex > 500) {
			...
			BasicStroke regularThickStroke = new BasicStroke(2.4f, BasicStroke.CAP_BUTT, BasicStroke.CAP_ROUND);
			pad.setStroke(regularThickStroke);
		}
		
		Font currentFont = pad.getFont();
		bigFont = currentFont.deriveFont(currentFont.getSize() * 1.9F);
		midFont = currentFont.deriveFont(currentFont.getSize() * 1.6F);
		pad.setFont(bigFont);
		...
		fontMetrics = frame.getFontMetrics(bigFont);
	...
	private void drawChartFrame() {
		...
		String title = "";
		/*
		 * if (type != null && type.equals(SMALL_TYPE)) { title = "Chart for " +
		 * stock.getCompanyName() + " (" + stock.getTicker() + "), " +
		 * SHORT_COPYRIGHT; } else { title = "Chart for " +
		 * stock.getCompanyName() + " (" + stock.getTicker() + "), " +
		 * REGULAR_COPYRIGHT; }
		 */
		...
		// pad.drawString("Daily Price Chart - " + currentDate, - origY +
		// BOX_GAP, VERTICAL_TEXT_X1);
	...
	private void drawBarChart() throws Exception {
		...
		for (int i = startIndex; i < priceList.size(); i++, xx += xStep) {
			...
			if (scanDateListHash.containsKey(daily.getMDate())) {
				// drawScanFlag(xx, (String)
				// scanDateListHash.get(daily.getMDate()));
			}
		...
		if (!hasVolumeError) {
			...
			pad.setColor(chartColor);
			pad.fillRect(origX + SMALL_GAP, volumeStrY - TEXT_HEIGHT + 3, volumeStrLen + 12, TEXT_HEIGHT + 5);
			pad.setColor(colorBlack);
			pad.drawRect(origX + SMALL_GAP, volumeStrY - TEXT_HEIGHT + 3, volumeStrLen + 12, TEXT_HEIGHT + 5);

			pad.setColor(colorBlue);
			pad.drawString(volumeStr, origX + SMALL_GAP + SMALL_GAP, volumeStrY + 2);
		}
	...
	private void displayMonthScale(Vector monthList) {
		...
		for (int i = 0; i < monthList.size(); i++) {
			...
			if (list != null && list.size() == 2) {
				...
				TextItem textItem = new TextItem(monthStr, (int) xxPLoc, (int) origY - SMALL_GAP);
	...
	private void displayEMAText() {
		...
		// backgroud
		pad.setColor(chartColor);
		pad.fillRect(emaStrX - 18, emaStrY - 15, totalLen, 27);
		pad.setColor(colorBlack);
		pad.drawRect(emaStrX - 18, emaStrY - 15, totalLen, 27);

		// 20-ema
		if (doEMA20) {
			pad.setColor(colorLtGreen);
			pad.fillRect(emaStrX - 12, emaStrY - 4, 6, 6);
			pad.drawString(emaStr, emaStrX, emaStrY + 6);
			emaStrX += strLen + 16;
		}
		...
		if (doEMA50) {
			pad.setColor(colorBlue);
			pad.fillRect(emaStrX - 12, emaStrY - 4, 6, 6);
			pad.drawString(emaStr2, emaStrX, emaStrY + 6);
			emaStrX += strLen2 + 16;
		}
		...
		if (doEMA100) {
			...
			pad.setColor(colorRed);
			pad.fillRect(emaStrX - 12, emaStrY - 4, 6, 6);
			pad.drawString(emaStr3, emaStrX, emaStrY + 6);
		}
	}
	...
	private void drawScanFlag(double xx, String scanKeyList) {
		...
		pad.setFont(new Font("TimesRoman", Font.PLAIN, 25));
	...
	private void prepareStockInfo() {
		...
		try {
			...
			ServiceContext svrContext = DBCachePool.getServiceContext(stock, period, displayDate, weekly, "N",
					stockDao);
			if (svrContext != null) {
	...
	public String getDrawGF() {
		return drawGF;
	}

	public void setDrawGF(String drawGF) {
		this.drawGF = drawGF;
	}
